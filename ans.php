...
$q2=mysqli_query($connect,"select * from  rt_forum where id='$catid'");
...
if(isset($btn))
{
	 ...
	 $mq=mysqli_query($connect,"select max(id) from rt_answer");
	 $mr=mysqli_fetch_array($mq);
	 $id=$mr['max(id)']+1;
	
	
	 $ins=mysqli_query($connect,"insert into rt_answer(id,postedby,category,question,answer,uname,rdate,catid) values($id,'$postedby','$category','$question','$answer','$uname','$rdate','$catid')");
	 if($ins)
	 {
	 ?>
                    ...
                    <form name="form1" method="post">
						<div class="row">
							...
							<div class="col-md-6 form-group">
								<label>Answer</label>
                                <textarea name="answer" class="form-control" cols="10" rows="5"></textarea>
							</div>
