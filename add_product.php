...
if(isset($btn))
{
	 $mq=mysqli_query($connect,"select max(id) from rt_product");
	 $mr=mysqli_fetch_array($mq);
	 $id=$mr['max(id)']+1;
	 $photo="";
	 if($_FILES['file']['name']!="")
	 {
	 $photo="P".$id.$_FILES['file']['name'];
	 move_uploaded_file($_FILES['file']['tmp_name'],"upload/".$photo);
	 }
	 
	
	 $ins=mysqli_query($connect,"insert into rt_product(id,retailer,category,product,price,quantity,photo,details,status,required_qty) values($id,'$uname','$category','$product','$price','$qty','$photo','$details','0','0')");
	 if($ins)
	 {
	 $msg="ok";
	 }
                    ...
                    <form name="form1" method="post" enctype="multipart/form-data">
						<div class="row">
							<div class="col-md-6 form-group">
								...
								<?php
								$cq=mysqli_query($connect,"select * from rt_category");
								while($cr=mysqli_fetch_array($cq))
								{
								?>
								<option><?php echo $cr['category']; ?></option>
								<?php
								}
								?>
								</select>
                        ...
                        <p></p>
                        <div>
                            <button class="btn btn-primary py-2 px-4" type="submit" name="btn">Add Product</button>
                        </div>
                    </form>
