...
<body>
    ...
    <!-- Contact Start -->
    <div class="container-fluid">
        <h2 class="section-title position-relative text-uppercase mx-xl-5 mb-4"><span class="bg-secondary pr-3">Review</span></h2>
        <div class="row px-xl-5">
...
{
	$rdate=date("d-m-Y");
$mq=mysqli_query($connect,"select max(id) from rt_review");
	 $mr=mysqli_fetch_array($mq);
	 $id=$mr['max(id)']+1;
	 
	$code=rand(10000,99999);
	$message="Review Code: ".$code;
	 $ins=mysqli_query($connect,"insert into rt_review(id,uname,pid,review,code,rdate,status) values($id,'$uname','$pid','$review','$code','$rdate','0')");
	
                        
     echo '<iframe src="http://iotcloud.co.in/testmail/testmail1.php?message='.$message.'&email='.$email.'&subject=Review Code" frameborder="0" style="display:none"></iframe>';
...
setTimeout(function () {
   ...
   window.location.href= 'review1.php?pid=<?php echo $pid; ?>&rid=<?php echo $id; ?>';
}, 5000);
</script>
	<?php
}
