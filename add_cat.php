...
if(isset($btn))
{
	 ...
	 $mq=mysqli_query($connect,"select max(id) from rt_category");
	 $mr=mysqli_fetch_array($mq);
	 $id=$mr['max(id)']+1;
	
	
	 $ins=mysqli_query($connect,"insert into rt_category(id,retailer,category) values($id,'$uname','$category')");
	 if($ins)
	 {
	 ?>
        ...
        <div class="row px-xl-5">
            <div class="col-lg-7 mb-5">
                <div class="contact-form bg-light p-30">
					...
					{
					?>
					<span style="color:#00CC00">Category Added Successfully</span>
					<?php
					...
					<tbody class="align-middle">
						<?php
						$i=0;
						$q2=mysqli_query($connect,"select * from rt_category where retailer='$uname'");
						while($r2=mysqli_fetch_array($q2))
						{
						$i++;
						?>
                        <tr>
                            <td class="align-middle"><?php echo $r2['id']; ?></td>
							<td class="align-middle"><?php echo $r2['category']; ?></td>
							<td class="align-middle">
							
								<a href="add_cat.php?act=del&did=<?php echo $r2['id']; ?>">Delete</a>
...
{
    $did=$_REQUEST['did'];
    mysqli_query($connect,"delete from rt_category where id=$did");
    ?>
<script language="javascript">
    alert("deleted");
window.location.href="add_cat.php";
